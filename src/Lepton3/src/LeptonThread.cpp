...
#include "ros/ros.h"
#include "lepton_msg_pkg/Lepton.msg"
...
{
	ros::NodeHandle nh;
	ros::Publisher pub = nh.advertise<lepton_msg_pkg::Lepton>("lepton", 10);
	lepton_msg_pkg::Lepton msg;
	
	ros::Rate r(1);
	
	//create the initial image
	myImage = QImage(myImageWidth, myImageHeight, QImage::Format_RGB888);
	uint8_t data[myImageWidth, myImageHeight] = {0,};
	...
	while(ors::ok()) {
		...
		for(int iSegment = iSegmentStart; iSegment <= iSegmentStop; iSegment++) {
			...
			for(int i=0;i<FRAME_SIZE_UINT16;i++) {
				...
				data[column][row] = valueFrameBuffer;
			}
		}
